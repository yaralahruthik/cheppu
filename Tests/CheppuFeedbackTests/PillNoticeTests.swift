...
/// Every state the Pill writes rather than draws, which is every state that has
/// words at all.
private let written: [PillState] =
...
struct PillNoticeTests {
    ...
    func everyNoticeFitsInsideThePill(_ state: PillState) throws {
        ...
        let room = PillPlacement.size.width - 2 * PillNotice.inset
