...
- What arrives is checked harder than FluidAudio checks it, not less hard: the repository's listing carries the Git LFS digest, which is the SHA-256 of the file's contents, so every large file is verified against it before it is given the name the Engine loads it by. Cheppu goes on to run those bytes as a model; the length agreeing is not enough.
- A finished download records what it fetched in a manifest beside the files. "Is the Engine here" is then answerable from disk, with no network and no guessing from the directory's shape — which would call an interrupted download finished, since a bundle's folder is made before the first byte of it arrives.
