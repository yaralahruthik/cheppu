...
Key-up is now watched as well as key-down, because a Hold on a chord ends when the key is let go of, and a Dictation that only stopped when the modifiers came up would run on past the words. It is the same question asked of the same field; a key-up that is neither Escape nor the Hotkey's is dropped where it arrives.
